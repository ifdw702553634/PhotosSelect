...
-(void)performDismiss:(NSTimer*)timer
{
    if ([timer userInfo]!=nil && [timer userInfo][@"completeBlock"]!=nil) {
        [self dismissViewControllerAnimated:YES completion:[timer userInfo][@"completeBlock"]];
    }
    else {
...
- (void)setDismissInterval:(CGFloat)interval complete:(void(^ __nullable)())complete;
{
    [NSTimer scheduledTimerWithTimeInterval:interval target:self selector:@selector(performDismiss:) userInfo:@{@"completeBlock" : complete} repeats:NO];
}
